...
overlap(v, w) = sum(((x, y),) -> conj(x) * y, zip(v, w))

# ╔═╡ cfbd957f-6caf-4f8a-af73-202b89e43705
let v = randn(5), w = randn(5)
	overlap(v, w), dot(v, w)
end

# ╔═╡ c65ae05c-fe55-4044-a2e3-3b53c6c5ed01
# return true if v and w are orthogonal, false otherwise
are_orthogonal(v, w) = overlap(v, w) == 0

# ╔═╡ 6360bb27-c25f-45da-93ba-85d74e77b7e7
# return norm of fector v
getnorm(v) = sqrt(real(overlap(v, v)))

# ╔═╡ 6855283c-3f9b-4d2c-9489-a6f809eb3cff
let v = randn(5)
	norm(v), getnorm(v)
end
...
@test mymul(σz, [0, 1]) ≈ [0, -1]

# ╔═╡ 9a02b122-7b62-48dd-a581-6b5a0ea81172
let A = randn(5, 5), v = randn(5)
	@test mymul(A, v) ≈ A * v
end
...
The measurement of an observeable will always return one of the eigenvalues of the hermitian matrix that represents that observeable. One example of this is the hydrogen atom, which has discrete energy levels, which correspond to the eigenvalues of the Hamiltonian describing the atom. When measuring the energy of an electron in the hydrogen atom, the result will always be one of the energy levels, never between them (barring measurement error). The probability of measuring a particular eigenvalue depends on the state of the system at the moment of measurement; It is given by the squared norm of the scalar product of the state with the corresponding eigenstate of the system. So if the initial state during measurement is $|ψ⟩$, and we want to know the probability of measuring the eigenvalue associated with the state $|↑⟩$, it is given by $P = ||⟨Ψ|↑⟩||^2$. After the measurement, the state of the system will be the eigenstate belonging to the eigenvalue that was returned by the measurement.

Numerically, we can compute the eigenvalues (`zvals`) and eigenvectors (`zvecs`) as below.
"

# ╔═╡ 63dbb47c-022b-468a-be97-7b2e01c7cffd
zvals, zvecs = eigen(σz)

# ╔═╡ 13e41ea1-ad64-46d3-9005-ba636b421eb3
md"This function performs a measurement of spin for a state `state` (this vector was defined in section 1 of the notebook) and returns either up (+1) or down (-1). Can you understand how it works?"

# ╔═╡ 7331fca3-7713-4e97-b442-14266d98b9c4
function zmeasure(state)
	pu = abs2(state[1])  # probability of up
	pd = abs2(state[2])  # probability of up
	# divide by sum to make sure probabilities sum to one
	if rand() < pu / (pu + pd)
		return 1
	else
		return -1
...
# ╔═╡ e5b7d61c-4279-404d-99d8-a7e51261f4eb
zmeasure([1, 1])
...
# ╔═╡ 57fe7150-daf0-4c1c-a549-770a9a932b4d
md"Now finish the function `measure` below, which performs a quantum measurement on a state using an arbitrary 2x2 matrix. You will likely need to use the expressions in the two cells above `measure`."

# ╔═╡ 27ce657b-f0aa-4026-8f67-50a8570ceab4
# get the sum of square absolute values or square norms
sum(abs2, [im, 3, 5 - im, 22])

# ╔═╡ 73feabb3-0a5c-437b-8bb9-ede9449bbd37
# get the square norm of the scalar product between the first eigenvector of σz and
# the state [1, 1]
abs2(dot(zvecs[:, 1], [1, 1]))

# ╔═╡ bb544d9e-9337-424d-8d63-54452b4dd6b0
function measure(observable, state)
	vals, vecs = eigen(observable)
	p1 = 0 # calculate probability of being in eigenstate 1
	if rand() < p1  # maybe normalise here by the sum of square absolute values
		return vals[1]
	else
		return vals[2]
	end
...
# ╔═╡ 33340aad-e77f-42ce-875d-90055d1d065d
md"
Write some code to test/play with the following questions below. Use your `measure` function.
1. What happens if you measure +1 in the z-direction, then measure in the x-direction and then measure in the z-direction again? Do you get +1 again? 
2. What happens if you try the same with different combinations of x, y and z measurements? 
3. If you measure +1 in the z-direction and the measure again, will the result change?
4. If you just measured spin in the z-direction and got +1, what is the probability of getting +1 or -1 in the x and y directions?
"

# ╔═╡ 406976c2-4049-4603-8774-221fc14e8a42
md"
## Expected Spin
The function below calculates the mean measured value by performing many repeated measurements and taking the average. Can you understand what it's doing?
"
...
function montecarlo_expect(observable, state, n::Integer)
	vals, vecs = eigen(observable)
	s = sum(abs2, state)
	p1 = abs2(dot(vecs[:, 1], state)) / s
	m = 0.0
	for i in 1:n
		# read the expression condition ? v : w as if condition, then v, else w
		m += (rand() < p1 ? vals[1] : vals[2])
		# these brackets actually aren't necessary, they just mark the whole
		# expression which is converted to a single value given to m += 
	end
	return m / n
end

# ╔═╡ 7b11092e-2478-42f1-bea8-25a800f43950


# ╔═╡ 7a13bc3f-dbb5-45c2-89a4-fcf723b8a488
# Should be right 99%+ of the time
@test montecarlo_expect(σz, [1, 1], 1_000) ≈ 0 atol=0.1 
#atol sets the tolerance of the ≈, so that it still accepts the values as being equal if the difference is 0.1. 
#The expectation value is zero, because we expect +1 50% of the time, and -1 50% of the time, so everything averages out. 

# ╔═╡ 54bc67ba-445c-4218-9faa-04dc53d698ad
md"The function below will also calculate the average. Does the Monte Carlo version above agree with the results of the below function? Check with some different states and observables."

# ╔═╡ ede610d1-1b22-474d-aabc-cec5b96102a3
expectation(A, state) = dot(state, A, state)
...
The propagator $U(t)$ of a quantum mechanical system is given by the time evolution operator $U(t) = exp(-iH ⋅ t/ħ)$. The propagator takes an intial state and returns the state after some time, so $ψ(t) = U(t) * ψ(0)$.
"

# ╔═╡ 5eb922d9-2ccd-4381-abf4-2d32a47fc83d
# This gives U(t) for a Hamiltonian H
propagator(H, t) = cis(-Hermitian(H)*t)

# ╔═╡ a4ca4561-e485-4e94-8d0c-73b47fe43959
@test propagator(H2, 0.123) * [1, 0] ≈ cis(0.123*2) * [1, 0]

# ╔═╡ 9a0c3438-4db4-4c0f-b50c-40f346e28310
md"Check whether the derivative of the state satisifes Schroedinger's equation $\frac{∂}{∂t}ψ(t) = -iH⋅ψ(t)$ by using the formula $\frac{∂}{∂t}ψ(t) ≈ \frac{ψ(t + dt) - ψ(t)}{dt}$ for a small $dt$. The formula is implemented below as `diff`."

# ╔═╡ 4420279f-41c4-40ab-8c52-ca90d33d22b0
diff(H, ψ0, t, dt) = (propagator(H, t + dt) * ψ0 - propagator(H, t) * ψ0) / dt

# ╔═╡ d288e492-fc0a-4904-9918-4d2595194bc4
diff(H2, [1, 0], 5, 0.001)

# ╔═╡ 8e0af6e4-4f8d-430b-837b-a5d8a0fc26db
-im * H2 * propagator(H2, 5) * [1, 0]

# ╔═╡ 73cf2ee5-1833-4854-9790-4e083ab2f911
md"Write a function `integrate` which proagates an intial state using monday's step by step method and returns the final state. The function takes a final time, an intial state, a number of steps n and a Hamiltonian H. Compare the result to suing the propagator above."

# ╔═╡ 0d34addb-ed2a-4b51-b32d-eb520914a77a


# ╔═╡ 0b51079d-e160-4a62-85ea-67d55a062ec9
function integrate(H, t, init, n)
	state = init  # initalise state
	dt = 0  # supposed to be time step to increment by each iteration
	# Change dt so that you reach the final time t in n steps
	for i in 1:n
...
# Gets polar and azimuth angle on sphere
...
# ╔═╡ 418c2db0-dde2-409e-8c37-cd0c6ed16474
md"To change the Hailtonian look at the top line of the plotting code below."
